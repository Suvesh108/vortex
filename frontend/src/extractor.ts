...
async function fallbackDirectStreamDownload(
  ...
  if (directUrl && directUrl.startsWith('http') && !directUrl.includes('youtube.com/watch')) {
    ...
    try {
      ...
      await saveFileToDevice(blob, filename, blobUrl, log);
      ...
      reader.onloadend = async () => {
        ...
        if (base64Data) {
          ...
          try {
            ...
            logger('success', `📁 File saved to: Internal Storage > Download > VortexDownloader > ${filename}`);
          } catch (err) {
            ...
            logger('success', `📁 File saved to: Internal Storage > Documents > VortexDownloader > ${filename}`);
          }
        }
      };
    } catch (e: any) {
